...
int main(int argc, char **argv)
{
  ...
  for(i=0;i<CatA.TotNgroups;i++)
	...
	factor = CatA.Group_M_Mean200[i]/pow(CatA.Group_R_Mean200[i],3);
      for(j=0;j<CatA.GroupNsubs[i]; j++)
	{
	  ...
	  if(k == CatA.GroupFirstSub[i])
	    ...
	    {
	      ahf_id.Rvir = pow(CatA.SubhaloMass[k]/factor,1./3.)*Mpc2kpc/a[snapid];
	    }
	  
	  ahf_id.Rmax = CatA.SubhaloVmaxRad[k]*Mpc2kpc/a[snapid];
